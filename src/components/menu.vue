<template>
  ...
  <!-- <el-radio-group v-model="isCollapse" style="margin-bottom: 20px">
    ...
    <el-menu
      ...
      @select="handleSelect"
    >
      <!-- <el-sub-menu :index="(index+1)+''" v-for="(item,index) in mess.data" :key="index+'1'">
        <template #title>
...
let mess = reactive({
  defaultActive:'1',
  isCollapse:false,
  data:["用户列表","文章列表","标签列表","首页展示","轮播展示","文章评论","首页评论"]
})

// onMounted(async () =>{ 
//   const {data:res} = await axios.get("/menus")
//   console.log(res) 
//   if(res.meta.status != 200) return ElMessage.error('菜单栏获取失败');
//   mess.data =  res.data  
// })
...
const handleSelect = (res) => {
  ...
  else if (res == '7') {router.push({path:'/indexComment'}) }
}
</script>
